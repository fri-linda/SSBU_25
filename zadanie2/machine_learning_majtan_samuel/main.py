    ...
    for metric in ["accuracy", "f1_score", "roc_auc", "precision"]:
        if metric in results.columns:
            plotter.plot_evaluation_metric_over_replications(
                results.groupby('model')[metric].apply(list).to_dict(),
                f"{metric.capitalize()} per Replication and Average {metric.capitalize()}",
                metric.capitalize()
            )
    plotter.plot_confusion_matrices(experiment.mean_conf_matrices)
    plotter.print_best_parameters(results)
    plotter.print_best_model(results, metric="f1_score")
