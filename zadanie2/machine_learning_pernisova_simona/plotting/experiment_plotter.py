...
class ExperimentPlotter(BasePlotter):
    ...
    def plot_metric_density(self, results, metrics=('accuracy', 'f1_score', 'roc_auc', 'recall'), save_path=None):
        """
        ...
        for metric in metrics:
            self._BasePlotter__generic_plot(
                ...
                ylabel='Density',
                figsize=(10, 6),
                save_path=os.path.join(save_path, f'density_{metric}.png') if save_path else None
            )

    def plot_evaluation_metric_over_replications(self, all_metric_results, title, metric_name, save_path=None):
        """
        ...
        self._BasePlotter__generic_plot(
            ...
            ylabel=metric_name,
            figsize=(10, 5),
            save_path=os.path.join(save_path, f'replications_{metric_name}.png') if save_path else None
        )

    def plot_confusion_matrices(self, confusion_matrices, save_path=None):
        """
        ...
        for model_name, matrix in confusion_matrices.items():
            self._BasePlotter__generic_plot(
                ...
                ylabel='True label',
                figsize=(6, 5),
                save_path=os.path.join(save_path, f'confusion_matrix_{model_name}.png') if save_path else None
            )
    ...
    def plot_evaluation_metric_over_replications_recall(self, all_metric_results, title, metric_name, save_path=None):
        self.plot_evaluation_metric_over_replications(all_metric_results, title, metric_name, save_path)