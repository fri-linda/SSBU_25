...
def plot_results(experiment, results, logger):
    ...
    save_path = "machine_learning"

    plotter.plot_metric_density(results, save_path=save_path)
    plotter.plot_evaluation_metric_over_replications(
        experiment.results.groupby('model')['accuracy'].apply(list).to_dict(),
        'Accuracy per Replication and Average Accuracy', 'Accuracy', save_path=save_path)
    plotter.plot_evaluation_metric_over_replications_recall(
        experiment.results.groupby('model')['recall'].apply(list).to_dict(),
        'Recall per Replication and Average Recall', 'Recall', save_path=save_path)
    plotter.plot_confusion_matrices(experiment.mean_conf_matrices, save_path=save_path)
