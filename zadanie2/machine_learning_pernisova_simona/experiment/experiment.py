...
class Experiment:
    ...
    def __initialize_csv_file(self):
     ...
     with open(self.accuracies_file, 'w') as file:
        file.write("Model,Replication,Accuracy,F1 Score,ROC AUC,Best Parameters,Recall\n")
    ...
    def __train_and_evaluate_model(self, model_name, X_resampled, y_resampled, replication):
        ...
        self.__store_results(model_name, replication, accuracy, f1, roc_auc, best_params, recall)
        self.replication_conf_matrices[model_name].append(confusion_matrix(y_test, predictions))

    def __store_results(self, model_name, replication, accuracy, f1, roc_auc, best_params, recall):
        ...
        with open(self.accuracies_file, 'a') as file:
            file.write(f"{model_name},{replication + 1},{accuracy:.4f},{f1:.4f},{roc_auc:.4f},\"{best_params}\",{recall:.4f}\n")
